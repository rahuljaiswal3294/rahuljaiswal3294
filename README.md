<!-- Header Section -->
<h1 align="center">Hi 👋, I'm Rahul Kumar Jaiswal</h1>
<h3 align="center">💻 Mobile Application Developer | React Native Specialist</h3>

<p align="center">
  📍 Khandwa, Madhya Pradesh, India | ✉️ <a href="mailto:rahuljaiswal3294@gmail.com">rahuljaiswal3294@gmail.com</a>  
  <br/>
  🔗 <a href="https://www.linkedin.com/in/rahul-kumar-jaiswal-216621177">LinkedIn</a> | 💻 <a href="https://github.com/rahuljaiswal3294">GitHub</a>
</p>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?size=22&duration=4000&color=36BCF7&center=true&vCenter=true&width=700&lines=Mobile+App+Developer+with+5%2B+Years+of+Experience;Android+and+React+Native+Expert;Passionate+about+Building+Scalable+and+Engaging+Apps" />
</p>

---

### 💼 Career Objective

Motivated and results-driven **Mobile Application Developer** with **5+ years** of experience in **React Native** development. Seeking to contribute to a forward-thinking organization where innovation, collaboration, and continuous learning drive success.

---

### 🧠 Technical Skills

| Category | Skills & Tools |
|-----------|----------------|
| **Programming** | React Native, JavaScript, HTML, XML |
| **Mobile Platforms** | Android (Java), React Native, Expo |
| **Tools & IDEs** | Android Studio, Visual Studio Code, Xcode, Postman |
| **Databases** | Firebase, SQLite |
| **Operating Systems** | Windows, Linux, macOS |
| **Version Control** | Git, GitHub |

---

### 🧑‍💻 Professional Experience

#### 🚀 **Protonshub Technologies** — React Native Developer  
*📅 Aug 2021 – Present | Indore, India*  
- Developed and maintained cross-platform apps using **React Native**.  
- Collaborated with UI/UX and backend teams for seamless performance.  
- Integrated **Firebase** and third-party APIs for real-time updates.  
- Led app performance optimization and code refactoring.

#### 💡 **ZeeweeSoft** — Android & React Native Developer  
*📅 Jan 2019 – Jul 2021 | Indore, India*  
- Built native **Android apps** in Java and transitioned to React Native.  
- Delivered multiple high-performance client apps with excellent UX.  
- Implemented unit testing and optimized app lifecycle management.

---

### 📱 Key Projects

| Project | Description | Tech Stack |
|----------|--------------|-------------|
| **Gazinta App** | A smart meat inventory app for hunters and homesteaders to track harvests, organize freezers, and reduce waste effortlessly. | React Native |
| **2NITE App** | Nightlife discovery app for Europe. | React Native |
| **Phil Ross Fitness** | Fitness & self-defense app with BodyBell Method®. | React Native |
| **UTS Live (Ultimate Tennis Showdown)** | Fan engagement app with live scores, social media integration & store. | React Native |
| **YLanes** | Audio/video conversation platform with curated topics. | React Native |
| **Incluzon** | Job placement app for graduates with assessments & learning. | React Native |
| **BookMySpot** | Appointment booking for salons, doctors, and services. | React Native |
| **Tara Gas** | Gas cylinder booking & accessories management app. | React Native |
| **The Home Game** | Poker game tracking app with live odds. | Expo |
| **Room-Check** | Hotel room management & service tracking. | Expo |
| **Ranky** | Fitness tracking app rewarding users via Google Fit. | Android (Java) |
| **Fat-food** | Homemade meal delivery & subscription app. | Android (Java) |
| **It’s My Society** | Society management app with polls and maintenance. | Android (Java) |

---

### 🎓 Education

- 🎓 **B.Tech in Instrumentation Engineering** — Bhaskaracharya College of Applied Science, Delhi University (2017)  
- 📘 **Higher Secondary (12th)** — Govt. Motilal Nehru H.S School, Khandwa (2011)  
- 📗 **Matriculation (10th)** — DAV Inter College School, Mau (2009)

---

### 🧩 Personal Details

- 🗓️ **Date of Birth:** 03 Feb 1994  
- 🗣️ **Languages:** English, Hindi  
- 🇮🇳 **Nationality:** Indian  
- 🌍 **Relocation:** Open  
- 💪 **Strengths:** Quick learner, self-motivated, detail-oriented  
- 🎶 **Hobbies:** Listening to music, playing chess  

---

### ⚙️ GitHub Analytics

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=rahuljaiswal3294&show_icons=true&theme=tokyonight" alt="rahuljaiswal3294 stats" width="48%"/>
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=rahuljaiswal3294&theme=tokyonight" alt="streak" width="48%"/>
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=rahuljaiswal3294&layout=compact&theme=tokyonight" width="50%"/>
</p>

---

### 🌐 Connect with Me

<p align="center">
  <a href="https://www.linkedin.com/in/rahul-kumar-jaiswal-216621177"><img src="https://skillicons.dev/icons?i=linkedin" width="45px" /></a>
  <a href="https://github.com/rahuljaiswal3294"><img src="https://skillicons.dev/icons?i=github" width="45px" /></a>
  <a href="mailto:rahuljaiswal3294@gmail.com"><img src="https://skillicons.dev/icons?i=gmail" width="45px" /></a>
</p>

---

<p align="center">💬 “Code. Create. Innovate.”</p>

---

⭐️ From [Rahul Kumar Jaiswal](https://github.com/rahuljaiswal3294)
